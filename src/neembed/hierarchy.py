...
HierarchyContract = Literal["tree", "dag"]
...
def _normalize_hierarchy_supervision(
    ...
    *,
    ...
    contract: HierarchyContract = "dag",
    ...
    if any(not isinstance(node_id, str) or not node_id for node_id in normalized_nodes):
    ...
    parents_by_child: dict[str, set[str]] = {node_id: set() for node_id in normalized_nodes}
    children_by_parent: dict[str, list[str]] = {node_id: [] for node_id in normalized_nodes}

    if isinstance(parent_child_edges, (str, bytes)):
        raise ValueError("parent_child_edges must contain (parent_id, child_id) pairs")
    for edge in parent_child_edges:
        if isinstance(edge, (str, bytes)) or not isinstance(edge, Sequence) or len(edge) != 2:
            raise ValueError("each hierarchy edge must be a (parent_id, child_id) pair")
        parent_id, child_id = edge
        if parent_id not in node_index or child_id not in node_index:
            raise ValueError("hierarchy edges must reference identifiers in node_ids")
    ...
    if contract == "tree" and any(len(parents) > 1 for parents in parents_by_child.values()):
        raise ValueError("tree hierarchy nodes may have at most one parent")

    _validate_acyclic_hierarchy(normalized_nodes, children_by_parent, parents_by_child)
    ...
    if roots is not None:
        ...
        if set(supplied_roots) != set(inferred_roots):
            raise ValueError("roots must match nodes with no incoming hierarchy edge")
        ...
        for parent_id, child_id in edges:
            if parent_id in depth_mapping and child_id in depth_mapping:
                if depth_mapping[parent_id] >= depth_mapping[child_id]:
                    raise ValueError("depth values must increase along hierarchy edges")
    ...
    normalized_edges = tuple(
        sorted(edges, key=lambda edge: (node_index[edge[0]], node_index[edge[1]]))
    )
