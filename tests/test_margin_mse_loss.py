"""Tests for teacher-scored manifold margin regression."""

import math

import neembed
import pytest
import torch
import torch.nn.functional as F
from torch import nn

import neembed.model as model_module
from neembed.losses import ManifoldMarginMSELoss
from neembed.model import ManifoldSentenceTransformer
from neembed.trainer import ManifoldTrainer


class FakeSentenceTransformer(nn.Module):
    """Small trainable encoder used to test losses without model downloads."""

    def __init__(self, model_name_or_path: str) -> None:
        super().__init__()
        self.model_name_or_path = model_name_or_path
        self.linear = nn.Linear(3, 4, bias=False)

    @property
    def device(self) -> torch.device:
        return self.linear.weight.device

    def get_embedding_dimension(self) -> int:
        return 4

    def preprocess(self, sentences: list[str]) -> dict[str, torch.Tensor]:
        rows = [[float(len(sentence)), 1.0, -1.0] for sentence in sentences]
        return {"input_features": torch.tensor(rows)}

    def forward(self, features: dict[str, torch.Tensor]) -> dict[str, torch.Tensor]:
        return {"sentence_embedding": self.linear(features["input_features"])}


class _LineManifold:
    def dist(self, a: torch.Tensor, b: torch.Tensor) -> torch.Tensor:
        return torch.abs(a - b).sum(dim=-1)


class _LookupModel(nn.Module):
    """Tiny deterministic model for exact teacher-margin cases."""

    def __init__(self) -> None:
        super().__init__()
        self.manifold = _LineManifold()
        self.values = {
            "anchor": 0.0,
            "close": 1.0,
            "far": 3.0,
        }

    def forward(self, sentences) -> torch.Tensor:
        return torch.tensor([[self.values[text]] for text in sentences])


def _make_model(
    monkeypatch,
    *,
    manifold: str = "poincare",
) -> ManifoldSentenceTransformer:
    monkeypatch.setattr(model_module, "SentenceTransformer", FakeSentenceTransformer)
    return ManifoldSentenceTransformer(
        "fake-model",
        manifold=manifold,
        embedding_dim=2,
    )


def test_margin_mse_loss_is_exported_from_package() -> None:
    assert neembed.ManifoldMarginMSELoss is ManifoldMarginMSELoss
    assert "ManifoldMarginMSELoss" in neembed.__all__


def test_margin_mse_sign_convention_matches_teacher_positive_minus_negative() -> None:
    model = _LookupModel()
    loss_fn = ManifoldMarginMSELoss(model=model)

    positive_preferred = loss_fn(
        ["anchor"],
        ["close"],
        ["far"],
        2.0,
    )
    negative_preferred = loss_fn(
        ["anchor"],
        ["far"],
        ["close"],
        -2.0,
    )

    assert positive_preferred.item() == pytest.approx(0.0)
    assert negative_preferred.item() == pytest.approx(0.0)


def test_margin_mse_has_controlled_nonzero_error() -> None:
    model = _LookupModel()
    loss_fn = ManifoldMarginMSELoss(model=model)

    loss = loss_fn(
        ["anchor"],
        ["close"],
        ["far"],
        1.0,
    )

    assert loss.item() == pytest.approx(1.0)


@pytest.mark.parametrize(
    "target_margin",
    [
        2.0,
        [2.0, 2.0],
        torch.tensor([2.0, 2.0]),
    ],
)
def test_margin_mse_accepts_scalar_list_and_tensor_targets(target_margin) -> None:
    model = _LookupModel()
    loss_fn = ManifoldMarginMSELoss(model=model)

    loss = loss_fn(
        ["anchor", "anchor"],
        ["close", "close"],
        ["far", "far"],
        target_margin,
    )

    assert loss.item() == pytest.approx(0.0)


def test_margin_mse_validates_aligned_non_empty_text_batches(monkeypatch) -> None:
    model = _make_model(monkeypatch)
    loss_fn = ManifoldMarginMSELoss(model=model)

    with pytest.raises(ValueError, match="must not be empty"):
        loss_fn([], [], [], 0.0)
    with pytest.raises(ValueError, match="anchors and positives"):
        loss_fn(["a"], ["p", "p2"], ["n"], 0.0)
    with pytest.raises(ValueError, match="anchors and negatives"):
        loss_fn(["a"], ["p"], ["n", "n2"], 0.0)
    with pytest.raises(ValueError, match="not a string"):
        loss_fn("anchor", ["p"], ["n"], 0.0)


@pytest.mark.parametrize(
    "target_margin",
    [
        [0.1],
        torch.tensor([[0.1, 0.2]]),
    ],
)
def test_margin_mse_requires_one_target_per_triplet_for_non_scalars(
    monkeypatch,
    target_margin,
) -> None:
    model = _make_model(monkeypatch)
    loss_fn = ManifoldMarginMSELoss(model=model)

    with pytest.raises(ValueError, match="scalar or one value per aligned triplet"):
        loss_fn(
            ["a1", "a2"],
            ["p1", "p2"],
            ["n1", "n2"],
            target_margin,
        )


@pytest.mark.parametrize(
    "target_margin, message",
    [
        (float("nan"), "finite"),
        (float("inf"), "finite"),
        (True, "real numeric"),
        (1.0 + 2.0j, "real numeric"),
        ("not-a-number", "real numeric"),
    ],
)
def test_margin_mse_rejects_invalid_target_values(
    monkeypatch,
    target_margin,
    message: str,
) -> None:
    model = _make_model(monkeypatch)
    loss_fn = ManifoldMarginMSELoss(model=model)

    with pytest.raises(ValueError, match=message):
        loss_fn(["a"], ["p"], ["n"], target_margin)


@pytest.mark.parametrize("manifold", ["poincare", "lorentz"])
def test_margin_mse_matches_direct_geodesic_formula(
    monkeypatch,
    manifold: str,
) -> None:
    torch.manual_seed(0)
    model = _make_model(monkeypatch, manifold=manifold)
    loss_fn = ManifoldMarginMSELoss(model=model)
    anchors = ["dog", "cat"]
    positives = ["mammal", "feline"]
    negatives = ["vehicle", "airplane"]
    target_margin = torch.tensor([0.25, -0.4])

    actual = loss_fn(anchors, positives, negatives, target_margin)

    anchor_embeddings = model(anchors)
    positive_embeddings = model(positives)
    negative_embeddings = model(negatives)
    predicted_margin = (
        model.manifold.dist(anchor_embeddings, negative_embeddings)
        - model.manifold.dist(anchor_embeddings, positive_embeddings)
    )
    expected = F.mse_loss(
        predicted_margin,
        target_margin.to(
            device=predicted_margin.device,
            dtype=predicted_margin.dtype,
        ),
    )

    assert torch.allclose(actual, expected)


@pytest.mark.parametrize("manifold", ["poincare", "lorentz"])
def test_margin_mse_is_finite_scalar_and_backpropagates(
    monkeypatch,
    manifold: str,
) -> None:
    torch.manual_seed(0)
    model = _make_model(monkeypatch, manifold=manifold)
    loss_fn = ManifoldMarginMSELoss(model=model)

    loss = loss_fn(
        ["dog", "cat"],
        ["mammal", "feline"],
        ["vehicle", "airplane"],
        [0.5, -0.25],
    )
    loss.backward()

    assert loss.shape == torch.Size([])
    assert torch.isfinite(loss)
    assert model.projection.weight.grad is not None
    assert model.encoder.linear.weight.grad is not None
    assert torch.isfinite(model.projection.weight.grad).all()
    assert torch.isfinite(model.encoder.linear.weight.grad).all()


def test_margin_mse_works_with_four_input_trainer_batch(monkeypatch) -> None:
    torch.manual_seed(0)
    model = _make_model(monkeypatch)
    loss_fn = ManifoldMarginMSELoss(model=model)
    trainer = ManifoldTrainer(model=model, loss=loss_fn, verbose=False)

    history = trainer.fit(
        [
            (
                ["dog", "cat"],
                ["mammal", "feline"],
                ["vehicle", "airplane"],
                torch.tensor([0.5, -0.25]),
            )
        ],
        epochs=1,
    )

    assert len(history) == 1
    assert math.isfinite(history[0])
